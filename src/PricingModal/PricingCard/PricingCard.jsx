...
const PricingCard = ({ title, price, feature1, feature2, feature3,recomendation,planid,advisor,days,fee }) => {
  const [modalIsOpen, setModalIsOpen] = useState(false);
  const navigate = useNavigate();

  const handleBuyNow = () => {
    navigate(`/payment/${advisor}/${planid}/${days}/${fee}`); // Navigate to the payment page
  };
  const openModal = () => {
    setModalIsOpen(true);
  };
