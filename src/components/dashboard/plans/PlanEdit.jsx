...
const EditPlan = () => {
  ...
  useEffect(() => {
    const fetchPlan = async () => {
      try {
        const response = await fetch(`https://localhost:8000/api/v1/advisor/get-plan-details/${edit}`, {
          method: 'GET',
    ...
    if (Object.keys(newErrors).length === 0) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          axios.patch(`https://localhost:8000/api/v1/advisor/edit-stocks/${edit}`, formData, { withCredentials: true })
            .then(response => {
